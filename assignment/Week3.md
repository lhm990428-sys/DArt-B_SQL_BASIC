...
- 사용한 집계 함수: max
- 집계 대상 컬럼:price
- 결과를 검증한 방법: price 컬럼의 값들 중 가장 큰 값이 출력되었는지 확인했다.
```

<img width="1917" height="1198" alt="image" src="https://github.com/user-attachments/assets/b4fd1884-6bb8-4e0a-82c4-9c3ce0293a4a" />
...
- 처음 틀렸다면 틀린 이유: count(dog, cat)처럼 count 안에 여러 값을 직접 넣으려고 했다.
- 새로 배운 SQL 패턴: where로 필요한 데이터 선택하고 group by로 그룹화 후 count로 개수 집계하는 법을 배웠다.
```

<img width="1917" height="1198" alt="image" src="https://github.com/user-attachments/assets/2e9cc601-d941-412a-9211-1baedc5e3222" />
...
```
1. 문제를 SQL로 옮길 때 가장 어려웠던 부분: 어떤 컬럼을 기준으로 묶고, 어떤 조건을 먼저 적용해야 하는지 판단하는 부분
2. WHERE와 HAVING의 차이를 어떻게 이해했는지: where는 그룹화 전에 데이터를 거르고, having은 그룹화한 결과에 조건을 거는 것으로 이해했다.
3. 다음 주에 더 연습하고 싶은 문제 유형: group by와 having을 함께 사용하는 집계 문제들과 쿼리문을 작성해나가는 순서를 배우고 싶다.
```
